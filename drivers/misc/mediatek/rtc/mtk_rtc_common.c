...
void rtc_mark_kernel_panic(void)
{
	unsigned long flags;

	rtc_xinfo("rtc_mark_kernel_panic\n");
	spin_lock_irqsave(&rtc_lock, flags);
	hal_rtc_set_spare_register(RTC_KERNEL_PANIC, 0x1);
	spin_unlock_irqrestore(&rtc_lock, flags);
}

void rtc_mark_kernel_restart(void)
{
	unsigned long flags;

	rtc_xinfo("rtc_mark_kernel_restart\n");
	spin_lock_irqsave(&rtc_lock, flags);
	hal_rtc_set_spare_register(RTC_KERNEL_RESTART, 0x1);
	spin_unlock_irqrestore(&rtc_lock, flags);
}
...
void rtc_mark_wdt_aee(void)
{
	...
	rtc_xinfo("rtc_mark_wdt_aee\n");
	spin_lock_irqsave(&rtc_lock, flags);
	hal_rtc_set_spare_register(RTC_WDT_AEE, 0x1);
	spin_unlock_irqrestore(&rtc_lock, flags);
}
